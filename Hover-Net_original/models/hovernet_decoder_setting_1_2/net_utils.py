...
class ResidualBlock(Net):
    ...
    def forward(self, prev_feat, freeze=False):
        if self.shortcut is None:
            ...
            shortcut = self.shortcut(prev_feat) # shortcut 또한 dimension을 위해 1x1 convolution 도입
        ...
        for idx in range(0, len(self.units)):
            ...
            if self.training:
                ...
                new_feat = self.units[idx](new_feat)
            prev_feat = new_feat + shortcut # 매 Units마다 Residual Connetion 진행
            shortcut = prev_feat
