...
def gen_instance_hv_map(ann, crop_shape):
    ...
    for inst_id in inst_list:
        ...
        # 1 ~ inst_map.shape[0] and 1 ~ inst_map.shape[1] to match the range of np.arange
...
def prep_sample(data, is_batch=False, nr_type=8, **kwargs):
    """
    Designed to process direct output from loader, adding tp_map visualization.
    """
    cmap = plt.get_cmap("jet")

    def colorize(ch, vmin, vmax, shape):
        ch = np.squeeze(ch.astype("float32"))
        ch = ch / (vmax - vmin + 1.0e-16)  # Normalize to 0-1
        ch_cmap = (cmap(ch)[..., :3] * 255).astype("uint8")  # Apply colormap
        ch_cmap = center_pad_to_shape(ch_cmap, shape)  # Pad to shape
        return ch_cmap
    ...
    def prep_one_sample(data):
        ...
        # HoVer Maps (Horizontal and Vertical)
        viz_list.append(colorize(data["hv_map"][..., 0] + 1, 0, 2, shape))
        viz_list.append(colorize(data["hv_map"][..., 1] + 1, 0, 2, shape))

        # True Positive (TP) Map - Uses discrete nr_type classes
        viz_list.append(colorize(data["tp_map"], 0, nr_type - 1, shape))
