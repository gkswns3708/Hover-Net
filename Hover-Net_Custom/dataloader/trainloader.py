...
class FileLoader(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        data = np.load(path)
        # print(path, "-FileLoader.__getitem_-path")
        ...
        # print(np.unique(ann[:, :, 1]), "- - train_loader.FileLoader.__getitem__-np.unique(ann)")
        ...
        # 이 시점에서는 np, hv map만 존재하는 것임. 이후에 loss 계산할 때 
        #  true_dict["tp"] = true_tp_onehot을 통해 NC branch의 label을 생성함
        target_dict = self.target_gen_func(
            inst_map, self.mask_shape, **self.target_gen_kwargs
        )
        feed_dict.update(target_dict)
        # print(np.unique(feed_dict['tp_map']), "- train_loader.FileLoader.__getitem__-np.unique(feed['tp_map'])")
        return feed_dict
