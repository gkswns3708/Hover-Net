...
class AccumulateRawOutput(BaseCallbacks):
    def run(self, state, event):
        ...
        # print(step_output.keys(), f'{os.path.realpath(__file__)}_step_output.keys()')
        accumulated_output = state.epoch_accumulated_output

        for key, step_value in step_output.items():
            # print(key, f'{os.path.realpath(__file__)}_key')
...
class BestModelSaver:
    """Callback for saving the model when the validation score improves."""
    def __init__(self, model, save_path, monitor='val_loss', mode='min', verbose=True):
        self.model = model
    ...
    def run(self, state, event):
        # Get validation score from state metrics
        current_score = state.metrics.get(self.monitor)
        
        # Initialize the best score if it's the first epoch
        if self.best_score is None:
            self.best_score = current_score
            self.save_model(state)
        
        # Check if the current score is better (depending on mode)
    ...
    def save_model(self, state):
        ...
        if self.verbose:
            print(f"Model saved to {self.save_path} with score: {self.best_score}")
