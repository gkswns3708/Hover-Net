...
def check_log_dir(log_dir):
    ...
    if os.path.isdir(log_dir):
        ...
        while True:
            ...
            key = input()
            print(key, f"- {os.path.relpath(__file__)}_key")
            if key == "d":
...
def save_image_from_batch(runs, imgs):
    # 첫 번째 이미지 선택
    img = imgs[0].cpu().detach().numpy()  # GPU -> CPU, Tensor -> NumPy 변환
    ...
    print(runs['net'], '- runs["net"]')
    if os.path.exists(f"./image_log/{runs['net']['run_time']}") == False:
        os.makedirs(f"./image_log/{runs['net']['run_time']}")
    save_path = f"./image_log/{runs['net']['run_time']}/batch_0_{time}.png"
